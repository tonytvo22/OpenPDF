...
public class PdfPKCS7 {
    ...
    private static final ASN1ObjectIdentifier AUTHORITY_INFO_ACCESS = new ASN1ObjectIdentifier("1.3.6.1.5.5.7.1.1");
    ...
    public PdfPKCS7(byte[] contentsKey, byte[] certsKey, String provider) {
        try {
            ...
            this.certs = readCertificates(new ByteArrayInputStream(certsKey));
            signCerts = this.certs;
            signCert = (X509Certificate) this.certs.iterator().next();
            crls = new ArrayList<>();
    ...
    private Collection<Certificate> readCertificates(ByteArrayInputStream certsKeyStream) throws CertificateException {
        CertificateFactory cf = new CertificateFactory();
        @SuppressWarnings("unchecked")
        Collection<Certificate> certificates = cf.engineGenerateCertificates(certsKeyStream);
        return certificates;
    }
    ...
    public PdfPKCS7(byte[] contentsKey, String provider) {
        try {
            ...
            certs = readCertificates(new ByteArrayInputStream(contentsKey));
            X509CRLParser cl = new X509CRLParser();
            cl.engineInit(new ByteArrayInputStream(contentsKey));
            @SuppressWarnings("unchecked")
            Collection<CRL> crls = (Collection<CRL>) cl.engineReadAll();
            this.crls = crls;
    ...
    public Certificate[] getSignCertificateChain() {
        return signCerts.toArray(new Certificate[signCerts.size()]);
    }
    ...
    public static String getOCSPURL(X509Certificate certificate)
            throws CertificateParsingException {
        try {
            String id = AUTHORITY_INFO_ACCESS.getId();
            ASN1Primitive obj = getExtensionValue(certificate, id);
            if (obj == null) {
                return null;
            }
